...
=head1 Virus Description Processor for GenBank Viruses

This is a subclass of L<VFile> that processes the C<checkv_genbank.tsv> file.  This file contains unknown viruses.  The name is in the
C<ncbi_name> field and the taxonomic ID is in C<ncbi_id>.

=head2 Special Methods

=head3 new

    my $vFile = VFile::GenBank->new($dbDir);
...
sub new {
    my ($class, $dbDir) = @_;
    my $retVal = {
        fileName => "$dbDir/genome_db/checkv_genbank.tsv"
    };
    bless $retVal, $class;
    return $retVal;
}
...
sub processHeader {
    my ($self, $ih) = @_;
    my (undef, $retVal) = P3Utils::find_headers($ih, virus_description => 'checkv_id', 'ncbi_id', 'ncbi_name');
    return $retVal;
}
